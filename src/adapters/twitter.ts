...
export class TwitterAdapter implements IAdapter {
  ...
  async connect(): Promise<void> {
    ...
    try {
      ...
      this.rwClient = this.twitterClient.readWrite ? this.twitterClient.readWrite : this.twitterClient;
  ...
  async sendMessage(_to: string, message: string): Promise<{ success: boolean; error?: string; code?: string }> {
    ...
    try {
      if (!this.rwClient) {
        return { success: false, error: "Twitter client not initialized", code: "TWITTER_CLIENT_NOT_INITIALIZED" };
      }
      ...
      if (typeof this.rwClient.v2?.tweet === "function") {
        ...
        return { success: false, code: "TWITTER_API_UNSUPPORTED", error: "Twitter API surface not supported in this runtime" };
      }
  ...
  async replyToMessage(to: string, message: string): Promise<{ success: boolean; error?: string; code?: string }> {
    ...
    try {
      ...
      if (typeof this.rwClient.v1?.reply === "function") {
        ...
        return { success: false, code: "TWITTER_API_UNSUPPORTED", error: "Twitter API surface not supported for replies" };
      }
      this.maybeDrainRate();
      return { success: true };
    } catch (e: any) {
