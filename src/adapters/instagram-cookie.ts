...
export class InstagramCookieAdapter implements IAdapter {
  ...
  private log(msg: string) { this.logger?.(`[InstagramCookie] ${msg}`); }
  ...
  async sendMessage(_to: string, message: string): Promise<{ success: boolean; error?: string; code?: string }> {
    ...
    try {
      ...
      return { success: false, error: e?.message ?? 'IG cookie post error', code: 'IG_COOKIE_POST_ERROR' };
    }
  }
  ...
  async replyToMessage(to: string, message: string): Promise<{ success: boolean; error?: string; code?: string }> {
    ...
    try {
      ...
      return { success: false, error: e?.message ?? 'IG cookie reply error', code: 'IG_COOKIE_REPLY_ERROR' };
    }
  }
