 ...
 *
 * Posts to Facebook using browser session cookies.
 * Cookies are passed as a string (plain "k=v; k2=v2") or JSON array
 * ([{"name":"k","value":"v"}]) — stored encrypted in the `accounts` table.
 *
 * This adapter calls Facebook's internal API endpoints, which don't require
 * an official access token. Use it for personal accounts without Graph API.
 *
 * IMPORTANT: Ensure you are compliant with Facebook's Terms of Service.
 */
export class FacebookCookieAdapter implements IAdapter {
  ...
  private log(msg: string) {
    this.logger?.(`[FacebookCookie] ${msg}`)
  }
  ...
  /**
   * Create a text-only Facebook post using the internal create endpoint.
   * @param _to  Unused (Facebook posts are to the authenticated user's feed)
...
export default FacebookCookieAdapter
