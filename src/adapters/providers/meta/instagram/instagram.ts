...
export class InstagramAdapter implements IAdapter {
  ...
  async connect(): Promise<void> {
    ...
    const id =
      (process.env as any).INSTAGRAM_BUSINESS_ACCOUNT_ID ||
      (cfg as any).INSTAGRAM_BUSINESS_ACCOUNT_ID
    if (!id) {
      this.log(
        'Instagram business account ID not configured (INSTAGRAM_BUSINESS_ACCOUNT_ID). Publish may fail without it.'
      )
    }
    this.igUserId = id
  ...
  async postMessage(
    ...
    if (!this.igUserId) {
      ...
      const id =
        (process.env as any).INSTAGRAM_BUSINESS_ACCOUNT_ID ||
        (getConfig() as any).INSTAGRAM_BUSINESS_ACCOUNT_ID
      if (!id) {
        return {
          success: false,
          error: 'IG user id not configured',
          code: 'INSTAGRAM_MISSING_IG_ID',
        }
      }
      this.igUserId = id
    }
