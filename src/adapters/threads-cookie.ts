...
export class ThreadsCookieAdapter implements IAdapter {
  ...
  private log(msg: string) { this.logger?.(`[ThreadsCookie] ${msg}`); }
    ...
    try {
      const client = createHttpClient({
        ...
        headers: {
          ...
          'User-Agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 17_0 like Mac OS X) AppleWebKit/605.1.15 Mobile/15E148 Instagram 303.0.0.11.109',
        },
      });
      ...
      return { success: false, error: e?.message ?? 'Threads cookie post error', code: 'THREADS_COOKIE_POST_ERROR' };
    }
  }
  ...
  async replyToMessage(to: string, message: string): Promise<{ success: boolean; error?: string; code?: string }> {
    ...
    try {
      ...
      return { success: false, error: e?.message ?? 'Threads cookie reply error', code: 'THREADS_COOKIE_REPLY_ERROR' };
    }
  }
  ...
  private maybeDrainRate() {
    const now = Date.now();
    if (now > this.rateReset) { this.rateRemaining = 30; this.rateReset = now + 60_000; }
