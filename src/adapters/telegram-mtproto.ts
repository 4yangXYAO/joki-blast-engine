...
export class TelegramMTProtoAdapter implements IAdapter {
  ...
  async connect(): Promise<void> {
    ...
    try {
      if (this.clientFactory) {
        ...
        const { TelegramClient, sessions: { StringSession } } = require('telegram');
  ...
  async sendMessage(to: string, message: string): Promise<{ success: boolean; error?: string; code?: string }> {
    if (!this.connected) {
      try {
        ...
        return { success: false, error: e?.message ?? 'Connection failed', code: 'MTPROTO_CONNECT_ERROR' };
      }
    }
    try {
      ...
      return { success: false, error: e?.message ?? 'Send failed', code: e?.code ?? 'MTPROTO_SEND_ERROR' };
    }
  }
  ...
  async replyToMessage(to: string, message: string): Promise<{ success: boolean; error?: string; code?: string }> {
    ...
    try {
      ...
      return { success: false, error: e?.message ?? 'Reply failed', code: e?.code ?? 'MTPROTO_REPLY_ERROR' };
    }
  }
