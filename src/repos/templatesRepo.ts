...
export class TemplatesRepo {
  ...
  create(input: { name: string; content: string; variables?: string[]; type: string }): Template {
    ...
    if (db.prepare && !isSqlJs) {
      db.prepare(`INSERT INTO templates (id, name, content, variables, type) VALUES (?, ?, ?, ?, ?)`).run(id, input.name, input.content, vars, input.type);
    } else {
      ...
      db.exec(`INSERT INTO templates (id, name, content, variables, type, created_at) VALUES ('${id}', '${safeName}', '${safeContent}', '${safeVars}', '${safeType}', datetime('now'))`);
    }
    return { id, ...input, created_at: new Date().toISOString(), updated_at: new Date().toISOString() };
  }

  list(): Template[] {
    const db = this.db ?? getDb();
    const rows = db.prepare("SELECT id, name, content, variables, created_at, updated_at, type FROM templates ORDER BY created_at DESC").all();
  ...
  findById(id: string): Template | null {
    const db = this.db ?? getDb();
    const r = db.prepare("SELECT id, name, content, variables, created_at, updated_at, type FROM templates WHERE id = ? LIMIT 1").get(id);
    if (!r) return null;
    return {
      id: r.id,
      name: r.name,
