...
import { computeBackoffDelay, isRetryableError } from "./retry";
      ...
      while (this.inMemoryQueue.length > 0) {
        ...
        try {
          ...
          if (err?.response?.status === 429) {
            ...
            console.debug(`[JobQueue] 429 Rate Limit hit for ${platform}. Blocking for ${retryAfter}s`);
            this.rateLimiter.blockFor(platform, retryAfter);
          }

          if (isRetryableError(err) && attempts < policy.maxRetries) {
            const delay = computeBackoffDelay(policy.baseDelay, policy.multiplier, attempts, policy.jitter);
            const nextItem = { ...item, attempts: attempts + 1 };
            console.debug(`[JobQueue] Retrying job ${item.id} in ${delay}ms (attempt ${attempts + 1})`);
            setTimeout(() => {
