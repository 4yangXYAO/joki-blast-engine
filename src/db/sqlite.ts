...
export async function initSqlJsDatabase(dbPath: string): Promise<DB> {
  ...
  // Provide a small wrapper to mimic better-sqlite3 API for exec/prepare/run
  // Do NOT alias wrapper to sqlDb to avoid accidental recursion when overriding methods.
  const originalExec = sqlDb.exec.bind(sqlDb);
  const originalPrepare = sqlDb.prepare.bind(sqlDb);
  const originalRun = (sql: string) => { try { return sqlDb.run(sql); } catch (e) { throw e; } };

  const wrapper: any = {
    exec: (sql: string) => originalExec(sql),
    prepare: (sql: string) => originalPrepare(sql),
    run: (sql: string) => originalRun(sql),
    export: () => sqlDb.export(),
    close: () => {
      // persist to disk
...
export function getDb(): DB {
  if (!_db) throw new Error("Database not initialized. Call initDatabase(path) or await initSqlJsDatabase(path) first.");
  return _db;
}
...
export function runMigrations(migrationsDir: string): void {
  ...
  for (const file of files) {
    try {
      ...
      if (!_isSqlJs) {
        const insert = db.prepare("INSERT INTO schema_migrations(name, applied_at) VALUES (?, datetime('now'))");
        ...
        db.exec(sql);
        db.exec(`INSERT INTO schema_migrations(name, applied_at) VALUES ('${file}', datetime('now'));`);
